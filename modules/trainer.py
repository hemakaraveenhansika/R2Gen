...
class BaseContrastiveTrainer(object):
    ...
    def train(self):
        ...
        print("start contrastive learn model train - lr_ve,lr_ed -> ", self.args.lr_ve, self.args.lr_ed)
...
class BaseR2GenTrainer(object):
    ...
    def train(self):
        not_improved_count = 0
        complete_reslts = {}
        print("start r2gen model train - lr_ve,lr_ed -> ", self.args.lr_ve, self.args.lr_ed)
        for epoch in range(self.start_epoch, self.epochs + 1):
            epoch_reslts = {}
            result = self._train_epoch(epoch)
